...
const Login = () => {
  ...
  const onChangeHandler = (e) => {
    setFormData({ ...formData, [e.target.name]: e.target.value }); // first destructure the formData and then update the corresponding value - dynamic
  };

  //---onsubmit handler----
  const onSubmitHandler = (e) => {
    e.preventDefault();
    dispatch(loginUserAction({ email, password })); // pass the action with payload
  };

  //get data from store
  const { error, loading, userInfo } = useSelector(
    (state) => state?.users?.userAuth // state - in callback function we have access to entire data in store (state) // optional chaining and returning the userAuth from the store which is destructured { error, loading, userInfo }
  );
